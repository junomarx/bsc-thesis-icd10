...
**Re-audited 9 August 2026 against the forward model and the `PROTOBASE-1.1`
localization correction.** The previous
...
day — `php vendor/bin/phpunit --testsuite unit,integration` then passed
237/237, and the E2E suite originally passed 7/7 (then 9/9 after the
first-visit tutorial and persisted-theme regressions; now 12/12 after the
complete bilingual audit) — so all five rows below now read
...
| `REQ-RUL-04` | ✅ Verified | `Rules/Precedence.php`'s `HARD_PRIORITY`/`GRADED_PRIORITY` constants implement `STATUS > DEPTH > EVID > REL-HARD` and `SPEC > REL-SPEC` deterministically regardless of input array order (`firstByPriority()` scans the fixed list, not the input). `app/tests/Unit/PrecedenceTest.php` (rewritten step 8, including vectors for the `REL-HARD-01`/`REL-SPEC-01` slots) passes in the current 83/83 unit suite. The localization correction changes explanation construction only, not these priorities. |
...
| `REQ-UI-03` | ✅ Verified | Every classification pairs an icon (`STATUS_ICONS`) with text — never colour alone; `:focus-visible` styling is global (`App.css`); the reduced-motion media query zeroes `--motion-duration` and the completion badge's `animation` explicitly, not just shortens it. The compact header `ThemeSwitch` has localized accessible/action text plus `aria-pressed`, applies the dark token palette to every view, and persists the selection under `icd10-prototype:theme`; `E2E/ThemeTest.php` verifies roster and tutorial colours, native `color-scheme`, reload persistence, and switching back without depending on the host preference. `PROTOBASE-1.1` adds strict `en-GB`/`de-AT` presentation: 349 inventoried entries, exact UI key/placeholder parity, complete 6/32/25 content and 87-code coverage, locale-aware document metadata, and Selenium traversal of the full learner surface in both locales (`LOCALIZATION_AUDIT.md`). A 375px-viewport Selenium pass showed no blocked control. No formal WCAG audit was performed (as the requirement itself allows — "no conformance claim unless separately tested"). |
...
| `REQ-CFG-01` | ✅ Verified | Step 10's original formal freeze (`docs/CONFORMANCE_REPORT.md` §1, `HANDOFF.md` §0.11) binds commit `298ab534e07caf87226a77e28c8a7734fe9dd8be` / `dev-freeze-4`, source checksum, `SUBSET-0.2`, `RULEBASE-0.2`, `RCBASE-0.3`, `PROTOBASE-1.0`, and all seven third-party image digests in `environment_manifest_0_1.json`. The post-freeze localization correction advances only the prototype identity to `PROTOBASE-1.1`; it preserves those semantic/source identifiers and image pins, records the inherited environment separately in `environment_manifest_0_2.json`, and is frozen at commit `8bb74b31596c3380c45b74643af63d744082be4e` / annotated tag `dev-freeze-5` with its own report/evidence package (`CONFORMANCE_REPORT_PROTOBASE_1_1.md`). The three project-owned GHCR tags remain deliberately mutable, so exact reproduction still requires building the cited source commit rather than pulling a later `:latest`. |
...
| `REQ-VER-04` | ✅ Verified | Rule/data/localization unit tests (`Unit/*`, 83 tests), persistence/API/evaluation integration (`Integration/*`, 163 tests, live MySQL), frontend localization-contract tests (4), and learner/progress/tutorial/theme/localization workflows (`E2E/*`, 12 tests, real Selenium) cover the forward model. Negative/boundary coverage includes malformed input, unsupported response kinds and gate failures; the bilingual additions cover all 143 oracle responses and every learner-visible patient/question/option path. Regression rerun after a material correction is demonstrated by the separate `PROTOBASE-1.1` report. |
...
**`PROTOBASE-1.0` remains an immutable historical freeze; `PROTOBASE-1.1` is
the localization-corrected revision** (`docs/CONFORMANCE_REPORT.md` and
`docs/CONFORMANCE_REPORT_PROTOBASE_1_1.md`, respectively). **What remains
before `REQBASE-1.0`/`TESTBASE-1.0`** (catalogue §12):
