...
| `REQ-CFG-01` | ✅ Verified | Step 10's original formal freeze (`docs/CONFORMANCE_REPORT.md` §1, `HANDOFF.md` §0.11) binds commit `298ab534e07caf87226a77e28c8a7734fe9dd8be` / `dev-freeze-4`, source checksum, `SUBSET-0.2`, `RULEBASE-0.2`, `RCBASE-0.3`, `PROTOBASE-1.0`, and all seven third-party image digests in `environment_manifest_0_1.json`. The post-freeze localization correction advances only the prototype identity to `PROTOBASE-1.1`; it preserves those semantic/source identifiers and image pins, records the inherited environment separately in `environment_manifest_0_2.json`, and has its own immutable tag/report/evidence package (`CONFORMANCE_REPORT_PROTOBASE_1_1.md`). The three project-owned GHCR tags remain deliberately mutable, so exact reproduction still requires building the cited source commit rather than pulling a later `:latest`. |
