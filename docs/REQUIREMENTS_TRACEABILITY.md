...
237/237, and the E2E suite originally passed 7/7 (now 9/9 after the
first-visit tutorial and persisted-theme regressions were added) — so all five rows below now read
...
| `REQ-CFG-01` | ✅ Verified | Step 10's formal freeze (`docs/CONFORMANCE_REPORT.md` §1, `HANDOFF.md` §0.11) binds every identifier this requirement asks for: git commit `298ab534e07caf87226a77e28c8a7734fe9dd8be` (tag `dev-freeze-4`, superseding `dev-freeze`/`dev-freeze-2`/`dev-freeze-3` — `docs/CONFORMANCE_REPORT.md` §9-§10); source catalogue checksum `diaglist_sha256 = 66713da5d63afcd37b0152ae7058f2188bf34d557bfa06ad4ce008825fb94a4b`; catalogue subset `SUBSET-0.2`; rule model `RULEBASE-0.2`; reference-case suite `RCBASE-0.3` (143 rows, `_candidate` designation dropped); test specification `TESTBASE-0.1` (as-built, §6 deviation note); software revision `PROTOBASE-0.3` → `PROTOBASE-1.0`. Execution-environment identification is bound alongside: `docs/environment_manifest_0_1.json` (frozen status) records the exact resolved version and manifest-list digest for **seven** container images — the original six plus `python:3.12-slim-bookworm` (`prototype_baseline/Dockerfile.bootstrap`'s base image, missed in the first pinning pass and corrected the same day, `docs/CONFORMANCE_REPORT.md` §9/`docs/DEVELOPMENT_DOCUMENTATION.md` §19.4) — and all seven digests are pinned directly in `Dockerfile`/`Dockerfile.bootstrap`/`docker-compose.yml`/`prototype_stack/compose.yaml`/`app/tests/E2E/docker-compose.yml`/`.github/workflows/ci.yml` on `master` (not just recorded in a side manifest). `develop` intentionally keeps floating tags for ongoing work (§19.2), which does not weaken this binding since `master` is the evaluated branch. **Explicitly not covered by this binding:** the three project-owned GHCR tags (`bsc-thesis-icd10:latest`/`:dev`, `bsc-thesis-icd10-bootstrap:latest`) stay mutable by design — reproducing this exact state requires building from source at the frozen commit, not pulling those tags (`docs/environment_manifest_0_1.json`'s `deliberately_not_recorded` note). |
