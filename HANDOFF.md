...
**Same-day revision note (fifth pass):** earlier versions of this file (preserved in git history) covered, in order: steps 1-6 plus the persistence/deployment-path fix (§0.1); then step 7 (`UXBASE-0.1` polish) plus the documentation-consolidation pass; then step 8 (§0.4) - `app/tests/*` rewritten wholesale against the patient/question model and genuinely passing; then CI's `push` trigger re-enabled after a stale-log false alarm (§0.5); then step 9, the oracle/source audit (§0.6) - all 129 previously-unaudited reference-response rows human-oracle-confirmed, zero discrepancies; **now: a repository housekeeping pass** (§0.7) - `prototype_baseline_0_2_design/` renamed to `prototype_baseline/`, `prototype_baseline_0_1/`/`development_handoff/`/`forward_package_0_6/` archived, and three real bugs this exercise surfaced (not invented for it) fixed along the way. Read §0 anyway — most of it still applies.
...
| 9 | Oracle/source audit reconciliation (125 new expectations + 4 provisional `VQ-005..008`) | Done — all 129 rows human-oracle-confirmed, zero discrepancies; see §0.6 |
...
**Step 10**: freeze + principal verification run. With step 9 done, nothing implementation-order-blocking remains before this. Concretely: settle `RCBASE-0.3`'s candidate status (drop the `_candidate` suffix once the project owner is ready to commit to it as frozen), settle the two supervisor-level open items (`OPEN-RQ-01`, `OPEN-EVAL-01`), then run the actual principal verification procedure `chapter3_test_catalogue.md` §3.2.2 describes and produce `REQ-VER-05`'s formal conformance report - this is a different, more formal thing than "the test suite passes," which is already true.

Separately, worth doing but not blocking: confirming the next push to `main` actually runs CI end-to-end (push trigger just re-enabled, §0.5) and passes now that both the bootstrap-wiring bug and the test suite itself are fixed - no GitHub-hosted run has completed against the fixed tree yet.
...
Selenium (project standard — **not Playwright**, see `CLAUDE.md`): `app/tests/E2E/docker-compose.yml`, per `app/tests/E2E/README.md`. The committed PHPUnit e2e suite now runs correctly (`--testsuite e2e`, 8/8, including the first-visit tutorial regression) against a stack started this way; for one-off ad hoc visual verification beyond what the committed suite covers, write a throwaway script against the same `php-webdriver/webdriver` dependency instead of reaching for anything else.
