...
| 10 | Freeze + principal verification run | **Done** — `PROTOBASE-1.0` frozen, zero defects found; see `docs/CONFORMANCE_REPORT.md` and §0.11 |
...
| Reference responses | `RCBASE-0.3` | **Frozen** — 143 rows (125 new + 18 historical), oracle-audited (step 9) and executed with zero defects in the principal verification run (§0.11); `_candidate` designation dropped from every file/identifier |
...
### 0.11 Step 10: the formal development freeze and principal verification run — `PROTOBASE-1.0`

Project-owner's five-point freeze instruction, executed in full: (1) pin the repository commit, container image digests, source catalogue checksum, and baseline identifiers; (2) execute the complete data-contract/persistence/unit/integration/reference-response/E2E/container-startup check battery in a clean environment; (3) record commands, environment versions, dates, counts, and results in a conformance report; (4) replace `_candidate` designations only after that run succeeds; (5) freeze the evidence package for Chapter 3 and the final evaluation chapter.

**Two corrections the project owner caught mid-task, both material:**
1. "Pin the repository commit, container image digests..." was initially misread as *recording* the six floating tags' resolved digests in a side manifest (§0.10's evidence, already done) without touching the actual `Dockerfile`/compose/CI files. The project owner caught this: a development freeze that leaves the shipped files on floating tags isn't frozen. Corrected by actually editing `Dockerfile`, `docker-compose.yml`, `prototype_stack/compose.yaml`, `app/tests/E2E/docker-compose.yml`, and `.github/workflows/ci.yml` to reference the six images by manifest-list digest (`@sha256:...`, the multi-arch-aware form — same rationale as §0.10/§10.7), not tag.
2. Pinning digests directly on the working branch would freeze ongoing development too, which isn't the intent — floating tags are genuinely useful during active work. The project owner proposed the fix: branch `develop` off the pre-pin commit, keeping its copy of every file above on floating tags for ongoing work, while the working branch (`master`) carries the pinned, frozen versions going forward. `develop` exists both locally and at `origin/develop`.

**Clean-environment principal verification run** — full account, commands, and exact figures: `docs/CONFORMANCE_REPORT.md`. Summary: pinned commit `7147b307caa6282766b2bed5a2d55c019d568937`; data-contract (4 checks), persistence (3 checks, fresh throwaway MySQL from the pinned digest), and the full containerized application suite (`docker compose --profile test run --rm test`: 77 unit + 160 integration + 9 E2E = 246/246, 2349 assertions) plus container-startup (5 checks) all run clean, then the application suite re-run in full a second time after the candidate-promotions below to confirm they introduced no regression — same 246/246 result both times. **258/258 discrete checks, exact conformance, zero defects found.** Per the instruction's own governing clause ("no further feature development shall be introduced unless the freeze run reveals an actual defect"), that zero-defect verdict means only naming/versioning promotions follow, no behavioural changes.

**Candidate designations dropped as a direct result of that verdict (point 4):** `reference_responses_0_3_candidate.csv` → `reference_responses_0_3.csv`; `oracle_manifest_0_3_candidate.json` → `oracle_manifest_0_3.json` (`status` → `frozen_predefined_expectations_at_protobase_1_0`); `rcbase_0_3_candidate_review.xlsx` → `rcbase_0_3_review.xlsx`; `docs/environment_manifest_0_1_candidate.json` → `docs/environment_manifest_0_1.json` (`status` → `frozen_at_protobase_1_0`); `prototype_baseline/persistence_candidate/candidate_file_digests.json` → `file_digests.json`; `runtime_manifest_0_2.json`'s `prototype_baseline_id` `PROTOBASE-0.3` → `PROTOBASE-1.0` (cascading, as it has each time this identity changed this session, into the canonical runtime digest — re-derived and re-pinned in `test_runtime_contract_0_2.py`, re-verified, not guessed) and its `status` → `frozen_evaluation_baseline`. Every PHP/Python docblock and path reference to the old `_candidate` names or `PROTOBASE-0.3` updated alongside (`app/src/Bootstrap.php`, `app/src/Config.php`, `app/tests/Integration/DatabaseTestCase.php`, `app/tests/Integration/ReferenceResponseTest.php`, `mysql_schema_0_2.sql`, `validate_forward_verification.py`).

**What this freezes, and what it deliberately does not** (full reasoning: `docs/CONFORMANCE_REPORT.md` §7): frozen — the commit, the six-image execution environment, the source-catalogue checksum, `RCBASE-0.3` (143 rows, no remaining reconciliation gap), and `PROTOBASE-0.3` → `PROTOBASE-1.0`. Not frozen, and out of scope for this report: `REQBASE-1.0` (gated on `OPEN-EVAL-01`, still a supervisor decision) and `TESTBASE-1.0` (the `TESTBASE-0.1` version-number gap noted since §0.8/§0.9 is a documentation gap for the thesis author, not a technical one this run found — §6 of the conformance report). `OPEN-RQ-01` and `OPEN-EVAL-01` remain exactly as unresolved as they have been throughout the project.

**Addendum, later the same day — a seventh image the pinning pass missed, found by project-owner review:** `prototype_baseline/Dockerfile.bootstrap` (the base of the published `bsc-thesis-icd10-bootstrap` image) still had `FROM python:3.12-slim-bookworm` floating even after the paragraphs above were written — it's one Dockerfile-reference-hop away from `docker-compose.yml`/the root `Dockerfile`/`ci.yml`, so the pinning pass's grep of those three never surfaced it. The "six-image execution environment" claimed as frozen above was six-out-of-seven. Corrected the same day: resolved and pinned its manifest-list digest (`python@sha256:4766d8b510c428e595d74b9cc5bbb2fae8e26316fffb4adc89908d79aacd58a2`, Python 3.12.13) the same way as the original six; added it to `docs/environment_manifest_0_1.json`; re-ran the full check battery against the corrected commit (fresh `freeze_evidence.zip`); and — per the same reasoning that ruled out amending the already-tagged, already-cited `dev-freeze` commit earlier in this session — cut a **new** immutable tag on the corrected commit rather than moving `dev-freeze`. Two smaller stale leftovers from the candidate-designation promotion above were fixed in the same pass: `.dockerignore`'s re-inclusion rule still named the pre-rename oracle CSV (confirmed harmless via an actual build, not assumed), and a cosmetic CI step name.

That tag (`dev-freeze-2`) turned out to have no CI run at all — trying to get one surfaced two further, independent bugs the same day: `.github/workflows/ci.yml`'s `on.push.branches` still read `[main]`, a branch renamed away to `master` at some earlier point, so every push since had silently never triggered CI; and, once that was fixed, `publish-images`'s own `if:` condition had the identical stale `main` reference in a second place, so it kept skipping even after CI started running again. Both fixed the same day, each its own tag (`dev-freeze-3`, then `dev-freeze-4`) rather than moving the one before it. **`dev-freeze-4` (commit `298ab534e07caf87226a77e28c8a7734fe9dd8be`) is the tag to cite for the original `PROTOBASE-1.0` lineage** — same 1.0 content as `dev-freeze-2`/`-3`, plus a CI pipeline that demonstrably runs and publishes correctly (first fully-green 5/5 run since the branch rename). Full account: `docs/CONFORMANCE_REPORT.md` §9-§10, `docs/DEVELOPMENT_DOCUMENTATION.md` §19.4, `docs/CHANGELOG.md`'s three same-dated entries.
